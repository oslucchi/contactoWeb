...
export default {
  ...
  props: {
    ...
    editEndpoint: { type: String, default: '' },
  ...
  methods: {
    ...
    buildPayload() {
      ...
      return { [this.colName]: this.localValue };
    },
    ...
    async onBlur() {
      ...
      try {
        ...
        this.$emit('saveCellConfirm', { id: this.rowId, col: this.colName, value: this.localValue, item: this.item, payload, response: res && res.data });
      } catch (err) {
  ...
  watch: {
    value(v) { 
      this.localValue = v == null ? '' : String(v); 
